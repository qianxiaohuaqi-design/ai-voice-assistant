...
const translateTarget = ref('Chinese')
...
const loadSettings = () => {
  ...
  anthropicBase.value = localStorage.getItem('anthropic_base') || ''
  ...
  translateTarget.value = localStorage.getItem('translate_target') || 'Chinese'
  ...
  isMuted.value = localStorage.getItem('is_muted') === 'true'
}
...
const saveSettings = () => {
  ...
  localStorage.setItem('is_muted', isMuted.value)
}

// Load settings immediately on module init (for SSR-safe access)
loadSettings()
